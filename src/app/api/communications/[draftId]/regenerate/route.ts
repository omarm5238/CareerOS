...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await regenerateCommunication(session.user.id, draftId.trim(), body);
    ...
    return NextResponse.json(
      { message: "Could not regenerate this communication. The previous revision is unchanged." },
      { status: 500 },
    );
  }
}
