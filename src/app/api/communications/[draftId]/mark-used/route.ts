...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await markCommunicationUsed(session.user.id, draftId.trim(), {
      recordTimelineEvent: (body as { recordTimelineEvent?: unknown }).recordTimelineEvent === true,
    });
    ...
    return NextResponse.json(
      { message: "Could not mark this communication as used." },
      { status: 500 },
    );
  }
}
