...
import { grantSubmissionApproval } from "@/features/application-execution/server";
...
export async function POST(_request: Request, context: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await grantSubmissionApproval(authResult.userId, sessionId));
  } catch (error) {
