...
import { toCareerMemoryErrorResponse } from "../errors";

export async function requireCareerMemoryUser() {
...
export function handleCareerMemoryError(error: unknown) {
  const mapped = toCareerMemoryErrorResponse(error);
